...
function Sidebar({ expanded: externalExpanded, setExpanded: setExternalExpanded }) {
  ...
  return (
    <>
      ...
      {expanded && (
        <div 
          className="fixed inset-0 bg-black/10 transition-all z-0"
          onClick={() => setExpanded(false)}
        />
      )}
      
      <aside className={`h-screen fixed left-0 top-0 transition-all duration-300 z-20 ${expanded ? "w-60" : "w-17"}`}>
        <nav className="h-full flex flex-col bg-white shadow-lg">
          ...
          <ul className="flex-1 px-3">
            ...
            <li className="relative hover:bg-deep-blue-100 rounded-md hover:-translate-y-0.5 hover:shadow-lg">
              <button
